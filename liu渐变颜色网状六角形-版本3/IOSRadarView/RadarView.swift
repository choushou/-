...
class RadarView: UIView {
    ...
    private var dataThree: [RadarModel]!
    private var dataFour: [RadarModel]!
    private var dataFive: [RadarModel]!
    ...
    private var reginLayerThree: CAShapeLayer!
    private var reginLayerFour: CAShapeLayer!
    private var reginLayerFive: CAShapeLayer!
    ...
    private var drawAreaColorThree: UIColor!
    private var drawAreaColorFour: UIColor!
    private var drawAreaColorFive: UIColor!
    
    
    private var dotRadiusTwo: CGFloat!
    private var dotColorTwo: UIColor!
    private var dotColorThree: UIColor!
    private var dotColorFour: UIColor!
    private var dotColorFive: UIColor!
...
extension RadarView {
    ...
    func initData(frame: CGRect) {
        ...
        drawAreaColor = UIColor.init(red: 1, green: 0, blue: 0, alpha: 1)
        dotColor = drawAreaColor
        
        drawAreaColorTwo = UIColor.init(red: 0, green: 0, blue: 0, alpha: 1)
        drawAreaColorThree = UIColor.init(red: 50, green: 100, blue: 100, alpha: 1)
        drawAreaColorFour = UIColor.init(red: 100, green: 200, blue: 100, alpha: 1)
        drawAreaColorFive = UIColor.init(red: 150, green: 200, blue: 100, alpha: 1)
        
        dotColorTwo = drawAreaColorTwo
        dotColorThree = drawAreaColorThree
        dotColorFour = drawAreaColorFour
        dotColorFive = drawAreaColorFive
    }
...
//                let x: CGFloat = (currentRadius * 0.5) * CGFloat(sin(angle + angle * CGFloat(node))) + centerX
        ...
        ///Three
        let percentPathThree = CGMutablePath()
              var arrayThree = [CGPoint]()
              for node in 0..<side {
                  let x: CGFloat = (radius - radiuSpace) * sin(angle  + angle * CGFloat(node)) * dataThree[node].percent + centerX
                  let y: CGFloat = (radius - radiuSpace) * cos(angle + angle * CGFloat(node)) * dataThree[node].percent + centerY
                  arrayThree.append(CGPoint(x: x, y: y))
              }
              percentPathThree.addLines(between: arrayThree)
        //Four
        let percentPathFour = CGMutablePath()
              var arrayFour = [CGPoint]()
              for node in 0..<side {
                  let x: CGFloat = (radius - radiuSpace) * sin(angle  + angle * CGFloat(node)) * dataFour[node].percent + centerX
                  let y: CGFloat = (radius - radiuSpace) * cos(angle + angle * CGFloat(node)) * dataFour[node].percent + centerY
                  arrayFour.append(CGPoint(x: x, y: y))
              }
              percentPathFour.addLines(between: arrayFour)
        //Five
        let percentPathFive = CGMutablePath()
              var arrayFive = [CGPoint]()
              for node in 0..<side {
                  let x: CGFloat = (radius - radiuSpace) * sin(angle  + angle * CGFloat(node)) * dataFive[node].percent + centerX
                  let y: CGFloat = (radius - radiuSpace) * cos(angle + angle * CGFloat(node)) * dataFive[node].percent + centerY
                  arrayFive.append(CGPoint(x: x, y: y))
              }
              percentPathFive.addLines(between: arrayFive)
        ...
        if reginLayerThree == nil {
            reginLayerThree = CAShapeLayer()
            reginLayerThree.fillColor = drawAreaColorThree.cgColor
            reginLayerThree.backgroundColor = UIColor.clear.cgColor
            reginLayerThree.path = percentPathThree
            reginLayerThree.lineWidth = lineWidth
            reginLayerThree.strokeColor = dotColorThree.cgColor
            reginLayerThree.frame = CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height)
            
            layer.insertSublayer(reginLayerThree, above: shapeLayer)
        ...
        if reginLayerFour == nil {
            reginLayerFour = CAShapeLayer()
            reginLayerFour.fillColor = drawAreaColorFour.cgColor
            reginLayerFour.backgroundColor = UIColor.clear.cgColor
            reginLayerFour.path = percentPathFour
            reginLayerFour.lineWidth = lineWidth
            reginLayerFour.strokeColor = dotColorFour.cgColor
            reginLayerFour.frame = CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height)
            
            layer.insertSublayer(reginLayerFour, above: reginLayerThree)
        ...
        if reginLayerFive == nil {
            reginLayerFive = CAShapeLayer()
            reginLayerFive.fillColor = drawAreaColorFive.cgColor
            reginLayerFive.backgroundColor = UIColor.clear.cgColor
            reginLayerFive.path = percentPathFive
            reginLayerFive.lineWidth = lineWidth
            reginLayerFive.strokeColor = dotColorFive.cgColor
            reginLayerFive.frame = CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height)
            
            layer.insertSublayer(reginLayerFive, above: reginLayerFour)
...
extension RadarView {
    ...
    func setDataThree(dataThree: [RadarModel]) {
         //        print("setData:\(data)")
         self.dataThree = dataThree
         self.side = self.dataThree.count
         // self.updateLayer()
     }
    func setDataFour(dataFour: [RadarModel]) {
         //        print("setData:\(data)")
         self.dataFour = dataFour
         self.side = self.dataFour.count
         // self.updateLayer()
     }
    func setDataFive(dataFive: [RadarModel]) {
         //        print("setData:\(data)")
         self.dataFive = dataFive
         self.side = self.dataFive.count
         // self.updateLayer()
     }
    ...
    func setDrawAreaColorThree(color: UIColor) {
        if color == nil {
            return
        }
        self.drawAreaColorThree = color
    }
    func setDrawAreaColorFour(color: UIColor) {
        if color == nil {
            return
        }
        self.drawAreaColorFour = color
    }
    func setDrawAreaColorFive(color: UIColor) {
        if color == nil {
            return
        }
        self.drawAreaColorFive = color
    }
